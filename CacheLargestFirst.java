...
public class CacheLargestFirst extends Cache{
	
	private HashMap<String, Data> map; //Bon pour facilement stocker et retrouver les valeurs
	private ArrayList<Data> cache; //Bon car dynamique
	private int XLRU;
	...
	public CacheLargestFirst(ArrayList<Data> c, HashMap<String, Data> mapLRU, int size, int warmup){
		super(size, warmup);
		this.map = mapLRU;
		this.cache = c;
		this.XLRU = 0;
	...
	public ArrayList<Data> getCache(){
		return this.cache;
	}
	...
	public void setCache(ArrayList<Data> newC) {
		this.cache = newC;
	}
	...
	public void setMap(HashMap<String, Data> newM) {
		this.map = newM;
	}
	...
	public int indexOfData (Data d){ //Pas grave car IndexOf est de complexite O(N)
		int r = -1;
	...
	public boolean containsData (Data d) { //Pas grave car Contains est de complexite O(N)
		boolean r = false;
		for (int i=0; i<this.getCache().size(); i++){
	...
	public void LargestCaching (Data newData) {
		...
		if (this.getMap().containsKey(newData.getUrl()) == true){ // Cas ou le cache contient la donnee
			...
			if (newData.getSize() == this.getCache().get(index).getSize()) { // Cas ou les donnees ont la meme taille
				...
				Data old = this.getCache().get(this.indexOfData(newData));
				...
				//Second case, there is no room for the new version of the data
					...
					Collections.sort(this.getCache());
					System.out.println(this.getCache());
					while((this.getCurrentSize()-old.getSize())+newData.getSize() > this.getN()){
				
						this.getMap().remove((this.getCache().get(0)).getUrl()); //Enleve le oldest element de la hashmap
						this.decrementCurrentSize(this.getCache().get(0).getSize()); //Remove number of bytes from the cache size
						this.getCache().remove(0); //Oldest = le dernier dans l'arraylist
